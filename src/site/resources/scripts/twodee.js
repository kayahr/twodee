...
twodee.SceneNode.prototype.updateTransformation=function(){var parentNode,transform,bounds;parentNode=this.parentNode;if(parentNode){this.effectiveTransform=parentNode.effectiveTransform.copy(this.effectiveTransform);transform=this.effectiveTransform.transform(this.transform)}else transform=this.effectiveTransform=this.transform.copy(this.effectiveTransform);bounds=this.bounds;if(bounds)bounds.setTransform(transform);return transform};
...
twodee.Physics.prototype.setMinVelocity=function(minVelocity){this.minVelocity=minVelocity};
twodee.Physics.prototype.getMinVelocity=function(){return this.minVelocity};
twodee.Physics.prototype.setMaxVelocity=function(maxVelocity){this.maxVelocity=maxVelocity};
...
twodee.Physics.prototype.setSpin=function(spin){this.spin=spin};
twodee.Physics.prototype.getSpinAcceleration=function(){return this.spinAcceleration};
twodee.Physics.prototype.setSpinAcceleration=function(spinAcceleration){this.spinAcceleration=spinAcceleration};
twodee.Physics.prototype.getMinSpin=function(){return this.minSpin};
twodee.Physics.prototype.setMinSpin=function(minSpin){this.minSpin=minSpin};
twodee.Physics.prototype.getMaxSpin=function(){return this.maxSpin};
twodee.Physics.prototype.setMaxSpin=function(maxSpin){this.maxSpin=maxSpin};
twodee.Physics.prototype.getScaling=function(){return this.scaling};
twodee.Physics.prototype.setScaling=function(scaling){this.scaling=scaling};
twodee.Physics.prototype.getLifetime=function(){return this.lifetime};
twodee.Physics.prototype.setLifetime=function(lifetime){this.lifetime=lifetime};
twodee.Physics.prototype.setDecay=function(decay){this.decay=decay};
...
twodee.Scene.prototype.rootNode=null;twodee.Scene.prototype.lastUpdate=0;twodee.Scene.prototype.collidables=null;twodee.Scene.prototype.paused=false;twodee.Scene.prototype.setRootNode=function(rootNode){this.rootNode=rootNode};
...
twodee.Scene.prototype.render=function(g,width,height){var node,i,collidables;if(!(node=this.rootNode))return;g.save();g.translate(width/2,height/2);this.renderNode(node,g);g.restore();collidables=this.collidables;for(i=collidables.length-1;i>=0;i--)collidables[i].processCollisions();collidables.length=0};
twodee.Scene.prototype.renderNode=function(node,g){var transform,child,i,other,collidables,next;if(!node.isEnabled())return;transform=node.updateTransformation();if(node.isCollidable()){collidables=this.collidables;for(i=collidables.length-1;i>=0;i--){other=collidables[i];if(node.collidesWith(other))node.collide(other);if(other.collidesWith(node))other.collide(node)}collidables.push(node)}g.save();g.globalAlpha=g.globalAlpha*node.getOpacity();node.render(g,transform);child=node.getFirstChild();while(child){next=
child.getNextSibling();this.renderNode(child,g);child=next}g.restore()};
